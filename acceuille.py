...
def show(app, navigate):
    ...
    def make_nav_btn(icon, main_text, sub_text="", active=False, command= None): # active is a boolean that indicates if this button is the currently active page
        txt=""
        if active:
            bg = "#f5c518"  
        else: 
        ...
        if command:
            BtnFrame.bind("<Button-1>", lambda e: command())
            for w in BtnFrame.winfo_children():
    ...
    btn_axe1 = make_nav_btn("⊘", "Axe 1", sub_text="Fonctions non linéaires", command=lambda: navigate("axe1"))
    btn_axe2 = make_nav_btn("▦", "Axe 2", sub_text="Systèmes linéaires", command=lambda: navigate("axe2"))
    btn_axe3 = make_nav_btn("📈", "Axe 3", sub_text="Interpolation &\nApproximation", command=lambda: navigate("axe3"))
    ...
    def make_axis_card(icon_file, title, subtitle, description, btn_color, hover, bg_color, command= None):
    ...
    # axe 1
    make_axis_card("fx.png", "Axe 1", "Fonctions non linéaires", "Résolution d'équations non linéaires et analyse de fonctions.","#f5c518", "#e6b800", "#ece8d8", command=lambda: navigate("axe1"))

    # axe 2
    make_axis_card("matrix.png", "Axe 2", "Systèmes linéaires", "Résolution de systèmes linéaires directs et itératifs.", "#4285f4", "#2a6dd9", "#cad9f3", command=lambda: navigate("axe2"))

    # axe 3
    make_axis_card("scatter.png", "Axe 3","Interpolation & Approximation", "Interpolation polynomiale, approximation et descente de gradient.", "#34a853", "#2a8a43", "#cdf1d7", command=lambda: navigate("axe3"))